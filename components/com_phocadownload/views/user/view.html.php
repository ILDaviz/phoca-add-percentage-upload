...
class PhocaDownloadViewUser extends JViewLegacy
{
	...
	function display($tpl = null) {
		...
		if($task == 'upload') {
			...
			$_SESSION['ft_errorfile_idcat'] = "";
			$_SESSION['ft_errorfile_email'] = "";
			$_SESSION['ft_errorfile_idweb'] = "";
			$_SESSION['ft_errorfile_format_size'] = "";
			$_SESSION['ft_errorfile_error_upload'] = "";
			...
			if ($post['catidfiles'] < 1) {
				$_SESSION['ft_errorfile_idcat'] = JText::_('COM_PHOCADOWNLOAD_PLEASE_SELECT_CATEGORY');
			...
			if ($post['email'] != '' && !JMailHelper::isEmailAddress($post['email']) ) {
				$_SESSION['ft_errorfile_email'] = JText::_('COM_PHOCADOWNLOAD_PLEASE_ENTER_VALID_EMAIL_ADDRESS');
			...
			if ($post['website'] != '' && !PhocaDownloadUtils::isURLAddress($post['website']) ) {
				$_SESSION['ft_errorfile_idweb'] = JText::_('COM_PHOCADOWNLOAD_PLEASE_ENTER_VALID_WEBSITE');
			...
			if(empty($fileArray)) {
				$_SESSION['ft_errorfile_format_size'] = JText::_('COM_PHOCADOWNLOAD_PLEASE_ADD_FILE_OR_IF_ADDED_CHECK_IF_IT_HAS_RIGHT_FORMAT_AND_SIZE');
			...
			if ($returnForm == 0) {
				...
				if($model->singleFileUpload($errorUploadMsg, $fileArray, $post)) {
					...
					$_SESSION['ft_errorfile_error_upload'] = JText::_('COM_PHOCADOWNLOAD_ERROR_FILE_UPLOADED');
					$this->t['errorfile'] = JText::_('COM_PHOCADOWNLOAD_ERROR_FILE_UPLOADED');
					if ($errorUploadMsg != '') {
						$_SESSION['ft_errorfile_error_upload'] = $errorUploadMsg;
	...
	/*	$state_files[] 		= JHTML::_('select.option',  '', '- '. JText::_( 'Select State' ) .' -' );
		...
		if ($this->tmpl['display_bootstrap3_layout'] == 1) {
			...
			$document->addScript(JURI::root(true).'/media/com_phocadownload/js/jquery.form.js');
